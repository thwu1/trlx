...
class MixtralModelBranch(transformers.PreTrainedModel):
    """
    ...
    def __init__(
        ...
        self.router_aux_loss_coef = base_model.model.config.router_aux_loss_coef
    ...
    def forward(
        ...
        if not return_dict:
            outputs = tuple(v for v in [hidden_states, next_cache, all_hidden_states, all_self_attns, all_router_logits] if v is not None)
        else:
...
# Copy MistralModelWithHydraValueHead and change mistral -> mixtral
class MixtralModelWithHydraValueHead(PreTrainedModelWrapper):
    ...
    def post_init(self, state_dict):
        ...
        if not self.peft_type and self.frozen_head is None:
            raise NotImplementedError("Loading a model trained with PPO requires `frozen_head` to be set.")
            for k in state_dict:
        ...
        for attn_mask, mask_name in zip([head_mask], ["head_mask"]):
            if attn_mask is not None:
                if attn_mask.size()[0] != (len(self.decoder_blocks)):
                    raise ValueError(
                        f"The `{mask_name}` should be specified for {len(self.decoder_blocks)} layers, but it is for" f" {head_mask.size()[0]}."
                    )
...
class LlamaModelBranch(ModelBranch):
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, hidden_states, past_key_values_length):
        ...
        combined_attention_mask = None
        if input_shape[-1] > 1:
            combined_attention_mask = self._make_causal_mask(input_shape, hidden_states.dtype, past_key_values_length=past_key_values_length).to(
...
def hf_get_branch_class(
    ...
    if arch in gpt_branch_supported_archs:
        ...
        raise ValueError(
            f"Unsupported architecture: `{arch}`. The following architectures are " f"available for model branching:\n{all_supported_archs}"
        )
